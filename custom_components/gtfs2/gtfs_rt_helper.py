...
def get_gtfs_feed_entities(url: str, headers, label: str):
    _LOGGER.debug(
        f"GTFS RT get_feed_entities for url: {url} , headers: {headers}, label: {label}"
    )
...
def get_next_services(self):
    ...
    _LOGGER.debug(
        "Configuration for RT route: %s, RT trip: %s, RT stop: %s, RT direction: %s",
        self._route,
        self._trip,
        self._stop,
        self._direction,
    ...
    next_services = (
        get_rt_route_trip_statuses(self)
        .get(self._route, {})
        .get(self._direction, {})
        .get(self._stop, {})
        .get("departures", [])
    ...
    if self._relative:
        due_in = due_in_minutes(next_services[0]) if len(next_services) > 0 else "-"
    else:
        due_in = dt_util.as_utc(next_services[0]) if len(next_services) > 0 else "-"
    ...
    if self._relative:
        ...
        attrs[ATTR_DEVICE_CLASS] = "timestamp" if len(next_services) > 0 else ""
...
def get_rt_route_trip_statuses(self):
    ...
    _LOGGER.debug(
        "Search departure times for route: %s, trip: %s, type: %s, direction: %s",
        self._route_id,
        self._trip_id,
        self._rt_group,
        self._direction,
    ...
    for entity in feed_entities:
        if entity.get("trip_update", False):
            ...
            if (
                self._rt_group == "route"
                and (route_id == self._route_id and direction_id == self._direction)
                or (trip_id == self._trip_id and direction_id == "nn")
            ) or (self._rt_group == "trip" and trip_id == self._trip_id):
                _LOGGER.debug(
                    "Entity found params - group: %s, route_id: %s, direction_id: %s, self_trip_id: %s, with rt trip: %s",
                    self._rt_group,
                    route_id,
                    direction_id,
                    self._trip_id,
                ...
                for stop in entity["trip_update"]["stop_time_update"]:
                    ...
                    if stop_id == self._stop_id or (
                        stop_id == "" and stop_sequence == self._stop_sequence
                    ):
                        ...
                        if (
                            direction_id == "nn"
                        ):  # i this case the trip_id serves as a basis so one can safely set direction to the requesting entity direction
                            direction_id = self._direction
                        ...
                        if stop["departure"].get("delay", 0) >= stop["arrival"].get(
                            "delay", 0
                        ):
                        ...
                        if due_in_minutes(datetime.fromtimestamp(stop_time)) >= 0:
                            departure_times[route_id][direction_id][stop_id][
                                ...
                                datetime.utcfromtimestamp(stop_time).replace(
                                    tzinfo=dt_util.get_time_zone("UTC")
                                )
                            )
                            _LOGGER.debug(
                                "RT stoptime: %s, utcfromtimestamp: %s, format utc: %s",
                                stop_time,
                                datetime.utcfromtimestamp(stop_time),
                                datetime.utcfromtimestamp(stop_time).replace(
                                    tzinfo=dt_util.get_time_zone("UTC")
                                ),
                            )
                            ...
                            _LOGGER.debug(
                                "Not using realtime stop data for old due-in-minutes: %s",
                                due_in_minutes(datetime.fromtimestamp(stop_time)),
                            )
...
def get_rt_vehicle_positions(self):
    ...
    for entity in feed_entities:
        ...
        if vehicle["trip"]["trip_id"] == self._trip_id:
            _LOGGER.debug(
                "Adding position for TripId: %s, RouteId: %s, DirectionId: %s, Lat: %s, Lon: %s, crc_trip_id: %s",
                vehicle["trip"]["trip_id"],
                vehicle["trip"]["route_id"],
                vehicle["trip"]["direction_id"],
                vehicle["position"]["latitude"],
                vehicle["position"]["longitude"],
                binascii.crc32((vehicle["trip"]["trip_id"]).encode("utf8")),
            )

        # add data if in the selected direction
        if (
            str(self._route_id) == str(vehicle["trip"]["route_id"])
            or str(vehicle["trip"]["trip_id"]) == str(self._trip_id)
        ) and str(self._direction) == str(vehicle["trip"]["direction_id"]):
            _LOGGER.debug("Found vehicle on route with attributes: %s", vehicle)
            _LOGGER.debug(
                "crc : %s", binascii.crc32((vehicle["trip"]["trip_id"]).encode("utf8"))
            )
            geojson_element = {
                "geometry": {"coordinates": [], "type": "Point"},
                "properties": {
            ...
            geojson_element["geometry"]["coordinates"].append(
                vehicle["position"]["longitude"]
            )
            geojson_element["geometry"]["coordinates"].append(
                vehicle["position"]["latitude"]
            )
            geojson_element["properties"]["id"] = (
                str(self._route_id)
                + "("
                + str(vehicle["trip"]["direction_id"])
                + ")"
                + str(binascii.crc32((vehicle["trip"]["trip_id"]).encode("utf8")))[-3:]
            )
            geojson_element["properties"]["title"] = (
                str(self._route_id)
                + "("
                + str(vehicle["trip"]["direction_id"])
                + ")"
            ...
            if entity.HasField("alert"):
                for x in entity.alert.informed_entity:
                    ...
                    if x.HasField("stop_id"):
                        ...
                        route_id = "unknown"
                if stop_id == self._stop_id and (
                    route_id == "unknown" or route_id == self._route_id
                ):
                    _LOGGER.debug(
                        "RT Alert for route: %s, stop: %s, alert: %s",
                        route_id,
                        stop_id,
                        entity.alert.header_text,
                    )
                    rt_alerts["origin_stop_alert"] = (
                        (str(entity.alert.header_text).split('text: "')[1])
                        .split('"', 1)[0]
                        .replace(":", "")
                        .replace("\n", "")
                    )
                if stop_id == self._destination_id and (
                    route_id == "unknown" or route_id == self._route_id
                ):
                    _LOGGER.debug(
                        "RT Alert for route: %s, stop: %s, alert: %s",
                    ...
                    rt_alerts["destination_stop_alert"] = (
                        (str(entity.alert.header_text).split('text: "')[1])
                        .split('"', 1)[0]
                        .replace(":", "")
                        .replace("\n", "")
...
def get_gtfs_rt(hass, path, data):
    ...
    try:
        ...
        if r.status_code != 200:
            _LOGGER.error(
                "Ìssues with downloading GTFS RT data, error: %s, content: %s",
                r.status_code,
                r.content,
            )
        ...
        _LOGGER.error(
            "Ìssues with downloading GTFS RT data to: %s", os.path.join(gtfs_dir, file)
        )
        return "no_rt_data_file"
    ...
    if data.get("debug_output", False):
        try:
            ...
            _LOGGER.info(
                "Ìssues with converting GTFS RT data to JSON, output to string"
            )
        # check if content is json else write without format
        try:
            open(os.path.join(gtfs_dir, file_all), "w").write(
                json.dumps(feed_entities, indent=4)
            )
    ...
    for ent in feed.entity:
        ...
        entity_dict = {
            "vehicle": {
                ...
                "vehicle": {"id": entity.vehicle.id, "label": entity.vehicle.label},
                "position": {
