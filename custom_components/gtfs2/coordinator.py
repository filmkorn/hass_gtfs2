...
from .gtfs_helper import (
    ...
    create_trip_geojson,
    check_extracting,
    get_local_stops_next_departures,
)
...
class GTFSUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self) -> dict[str, str]:
        ...
        self._pygtfs = get_gtfs(self.hass, DEFAULT_PATH, data, False)
        ...
        if check_extracting(self.hass, self._data["gtfs_dir"], self._data["file"]):
            _LOGGER.debug(
                "Cannot update this sensor as still unpacking: %s", self._data["file"]
            )
        ...
        if previous_data is not None and (
            datetime.datetime.strptime(
                previous_data["gtfs_updated_at"], "%Y-%m-%dT%H:%M:%S.%f%z"
            )
            + timedelta(
            ...
            run_static = False
            _LOGGER.debug(
                "No run static refresh: sensor exists but not yet refresh for name: %s",
                data["name"],
            )
        # 2. sensor exists and refresh interval reached, get static data
            ...
            run_static = True
            _LOGGER.debug(
                "Run static refresh: sensor without gtfs data OR refresh for name: %s",
                data["name"],
            )
        ...
        if not run_static:
            ...
            try:
                ...
                return None
            _LOGGER.debug(
                "GTFS coordinator data from helper: %s", self._data["next_departure"]
            )
        ...
        if "real_time" in options:
            if options["real_time"]:
                ...
                if self._route_id == None:
                    _LOGGER.debug(
                        "GTFS RT: no route_id in sensor data, using route_id from config_entry"
                    )
                    self._route_id = data["route"].split(": ")[0]
                self._stop_id = self._data["next_departure"]["origin_stop_id"].split(
                    ": "
                )[0]
                self._stop_sequence = self._data["next_departure"][
                    "origin_stop_sequence"
                ]
                self._destination_id = data["destination"].split(": ")[0]
                self._trip_id = self._data.get("next_departure", {}).get(
                    "trip_id", None
                )
                self._direction = data["direction"]
                self._relative = False
                try:
                    self._get_rt_alerts = await self.hass.async_add_executor_job(
                        get_rt_alerts, self
                    )
                    self._get_next_service = await self.hass.async_add_executor_job(
                        get_next_services, self
                    )
                    ...
                    _LOGGER.error(
                        "Error getting gtfs realtime data, for origin: %s with error: %s",
                        data["origin"],
                        ex,
                    )
    ...
    async def _async_update_data(self) -> dict[str, str]:
        ...
        if "real_time" in options:
            if options["real_time"]:
                ...
                if options.get(CONF_API_KEY_LOCATION, None) == "header":
                    self._headers = {options[CONF_API_KEY_NAME]: options[CONF_API_KEY]}
                    self._headers[CONF_API_KEY_LOCATION] = options.get(
                        CONF_API_KEY_LOCATION, None
                    )
                    self._headers[CONF_API_KEY_NAME] = options.get(
                        CONF_API_KEY_NAME, None
                    )
                    self._headers[CONF_API_KEY] = options.get(CONF_API_KEY, None)
                    self._headers[CONF_ACCEPT_HEADER_PB] = options.get(
                        CONF_ACCEPT_HEADER_PB, False
                    )
