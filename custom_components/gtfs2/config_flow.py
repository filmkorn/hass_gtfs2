...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_local_stops(
                ...
                data_schema=vol.Schema(
                    {
                        vol.Required(CONF_ROUTE_TYPE): selector.SelectSelector(
                            selector.SelectSelectorConfig(
                                options=["99", "2"], translation_key="route_type"
                            )
                        ),
                    },
                ),
    ...
    async def async_step_route(self, user_input: dict | None = None) -> FlowResult:
        ...
        if user_input is None:
            ...
            return self.async_show_form(
                ...
                data_schema=vol.Schema(
                    {
                        vol.Required(CONF_ROUTE, default=""): selector.SelectSelector(
                            selector.SelectSelectorConfig(
                                options=route_list,
                        ...
                        vol.Required(CONF_DIRECTION): selector.SelectSelector(
                            selector.SelectSelectorConfig(
                                options=["0", "1"], translation_key="direction"
                            )
                        ),
                    },
                ),
                    ...
                    data_schema=vol.Schema(
                        {
                            ...
                            vol.Required(CONF_DESTINATION, default=last_stop): vol.In(
                                stops
                            ),
                            vol.Required(CONF_NAME): str,
                            vol.Optional(
                                CONF_INCLUDE_TOMORROW, default=False
    ...
    async def async_step_stops_retry(
        self, user_input: dict | None = None
    ) -> FlowResult:
        """Handle the route step."""
        errors: dict[str, str] = {}
            ...
            try:
                ...
                return self.async_show_form(
                    ...
                    data_schema=vol.Schema(
                        {
                            vol.Required(
                                CONF_ORIGIN, default=self._user_inputs[CONF_ORIGIN]
                            ): vol.In(stops),
                            vol.Required(
                                CONF_DESTINATION,
                                default=self._user_inputs[CONF_DESTINATION],
                            ): vol.In(stops),
                            vol.Required(CONF_NAME): str,
                            vol.Optional(
                                CONF_INCLUDE_TOMORROW, default=False
    ...
    async def async_step_stops_train(
        self, user_input: dict | None = None
    ) -> FlowResult:
    ...
    async def async_step_init(
        ...
        if user_input is not None:
            ...
            if user_input.get(CONF_REAL_TIME, None):
                self._user_inputs.update(user_input)
                _LOGGER.debug(
                    f"UserInputs Options Init with realtime: {self._user_inputs}"
                )
                return await self.async_step_real_time()
            else:
                self._user_inputs.update(user_input)
                _LOGGER.debug(
                    f"UserInputs Options Init without realtime: {self._user_inputs}"
                )
        ...
        if self.config_entry.data.get(CONF_DEVICE_TRACKER_ID, None):
            opt1_schema = {
                ...
                vol.Optional(
                    CONF_RADIUS,
                    default=self.config_entry.options.get(
                        CONF_RADIUS, DEFAULT_LOCAL_STOP_RADIUS
                    ),
                ): vol.All(vol.Coerce(int), vol.Range(min=50, max=5000)),
                vol.Optional(
                    CONF_TIMERANGE,
                    default=self.config_entry.options.get(
                        CONF_TIMERANGE, DEFAULT_LOCAL_STOP_TIMERANGE
                    ),
                ...
                vol.Optional(
                    CONF_REAL_TIME,
                    default=self.config_entry.options.get(CONF_REAL_TIME),
                ): selector.BooleanSelector(),
            }
            return self.async_show_form(
                step_id="init", data_schema=vol.Schema(opt1_schema), errors=errors
            )
            ...
            opt1_schema = {
                ...
                vol.Optional(
                    CONF_SOURCE_TIMEZONE_CORRECTION,
                    default=self.config_entry.options.get(
                        CONF_SOURCE_TIMEZONE_CORRECTION, 0
                    ),
                ): int,
                vol.Optional(
                    CONF_REAL_TIME,
            ...
            return self.async_show_form(
                step_id="init", data_schema=vol.Schema(opt1_schema)
            )
                ...
                data_schema=vol.Schema(
                    {
                        vol.Required(
                            CONF_TRIP_UPDATE_URL,
                            default=self.config_entry.options.get(CONF_TRIP_UPDATE_URL),
                        ): str,
                        vol.Optional(
                            CONF_API_KEY,
                ...
                data_schema=vol.Schema(
                    {
                        ...
                        vol.Optional(
                            CONF_ALERTS_URL,
                            default=self.config_entry.options.get(CONF_ALERTS_URL, ""),
                        ): str,
                        vol.Optional(
                            CONF_API_KEY,
                            default=self.config_entry.options.get(CONF_API_KEY),
